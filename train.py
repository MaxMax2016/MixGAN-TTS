    ...
    while True:
        ...
        for batchs in loader:
            for batch in batchs:
                ...
                losses = [D_loss + G_loss, D_loss, G_loss, recon_loss, fm_loss, adv_loss, mel_loss, postnet_loss, pitch_loss, energy_loss, duration_loss, helper_loss]
                losses_msg = [D_loss + G_loss, D_loss, adv_loss, mel_loss, postnet_loss, pitch_loss, energy_loss, duration_loss, helper_loss]

                if step % log_step == 0:
                    losses_msg = [sum(l.values()).item() if isinstance(l, dict) else l.item() for l in losses_msg]
                    message1 = "Step {}/{}, ".format(step, total_step)
                    message2 = "Total Loss: {:.4f}, D_loss: {:.4f}, adv_loss: {:.4f}, mel_loss: {:.4f}, postnet_loss: {:.4f}, pitch_loss: {:.4f}, energy_loss: {:.4f}, duration_loss: {:.4f}, helper_loss:{:.4f}".format(
                        *losses_msg
                    )
                ...
                if step % synth_step == 0:
                    figs, attn_fig, wav_reconstruction, wav_prediction, tag = synth_one_sample(
                        args,
                        batch,
                        output,
                        coarse_mels,
