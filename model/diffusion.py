...
class GaussianDiffusion(nn.Module): # 高斯扩散降噪
    def __init__(self, args, preprocess_config, model_config, train_config):
        super().__init__()
        self.model = args.model
        self.denoise_fn = Denoiser(preprocess_config, model_config)         # 扩散降噪解码器
    ...
    @torch.no_grad()
    def sampling(self, noise=None): # 去噪
        ...
        for i in tqdm(reversed(range(0, t)), desc="sample time step", total=t):
            ...
            xs.append(x)
        output = [self.denorm_spec(x[:, 0].transpose(1, 2)) for x in xs]    # 反正则化
        return output

    def diffuse_trace(self, x_start, mask): # (8,x,80), (8,x)
        b, *_, device = *x_start.shape, x_start.device  # int(8), list:2 [x,80], device:cuda:0
        trace = [self.norm_spec(x_start).clamp_(-1., 1.) * ~mask.unsqueeze(-1)] # list:2   [ (8,x,80) * (8,x,1) = (8,x,80) ]
    ...
    def diffuse_fn(self, x_start, t, noise=None):       # 可能是diffusion过程
    ...
    def forward(self, mel, cond, spk_emb, mel_mask, coarse_mel=None, clip_denoised=True):
        ...
        if mel is None:                 # 推理阶段
            ...
            # Diffusion
            x_t = self.diffuse_fn(mel, t) * mel_mask    # 从x_0得到x_t
            x_t_prev = self.diffuse_fn(mel, t - 1) * mel_mask       # x_{t-1}

            # Predict x_{start}
            x_0_pred = self.denoise_fn(x_t, t, cond, spk_emb) * mel_mask    # x_o'
            if clip_denoised:
                x_0_pred.clamp_(-1., 1.)
            ...
            if self.model != "shallow":
                ...
                x_start = x_start.transpose(1, 2)[:, None, :, :]  # [B, 1, M, T]
            x_t_prev_pred = self.q_posterior_sample(x_start=x_start, x_t=x_t, t=t) * mel_mask  # 得到x_{t-1}'
