...
def squeeze(x, x_mask=None, g=None, n_sqz=2):
    x, x_mask, g, pad_size = pad_input(x, x_mask, g, n_sqz) # (8,80,x), (8,1,x), (8,192,x), (0)
    b, c_x, t_x = x.size()
    t_x = (t_x // n_sqz) * n_sqz
    x = x[:, :, :t_x]
...
class FlowPostNet(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config):
        ...
        shared_group = model_config["postnet"]["shared_group"]  # 3

        self.in_channels = in_channels
        self.n_sqz = n_sqz
        self.flows = nn.ModuleList()
        for b in range(n_blocks):
            create_wn = b % shared_group == 0       # 当b = 1,2时，create_wn = 0,则self.wn = flows[..]
            if create_wn:                           # 当b = 3时，create_wn = 1,则self.wn = NonCausalWaveNet(..)
                shared_wn_idx = b
            self.flows.append(ActNorm(channels=in_channels * n_sqz))        # 初始化参数  累加  四则运算计算对数似然
            self.flows.append(InvConvNear(                                  # 看不懂    可能为可逆卷积
    ...
    def forward(self, x, x_mask, g=None):
        ...
        return x.transpose(1, 2), logdet_tot    # (8,80,40), tensor(8)
    ...
    def inference(self, x_mask, g=None, temperature=0.8):
        ...
        if self.n_sqz > 1:
            x, g, x_mask, pad_size = squeeze(x, x_mask, g, self.n_sqz)
        for f in flows:
            x, logdet = f(x, x_mask, g=g, reverse=True) # (1,160,0) (1,1,0) (1,512,0)
        if self.n_sqz > 1:
            x, g, x_mask = unsqueeze(x, x_mask, g, self.n_sqz, pad_size)
...
class CouplingBlock(nn.Module):
    def __init__(self, in_channels, hidden_channels, kernel_size, dilation_rate, n_layers, gin_channels=0, p_dropout=0, sigmoid_scale=False, n_sqz=1, shared_wn=None):
        ...
        # Grouped Parameter Sharing     # 分组参数共享机制
